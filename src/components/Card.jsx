...
const Card = ({ farm, onClick }) => {
  return (
    <div
      className="relative w-72 cursor-pointer overflow-hidden rounded-lg bg-white shadow-lg transition-shadow duration-300 hover:shadow-xl"
      onClick={onClick}
    >
      {/* 이미지 */}
      <div className="h-48 overflow-hidden">
        <img
          src={farm.image_url || "https://via.placeholder.com/150"}
          alt={farm.title}
      ...
      <div className="border-t p-4">
        <button className="w-full rounded-lg bg-blue-500 py-2 text-center text-white hover:bg-blue-600">
          상세 보기
        </button>
      </div>
    </div>
